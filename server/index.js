...
app.post('/api/projects/:id/audio', authenticateToken, uploadAudio.single('audio'), async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            // Wrap in promise to await execution
            const transcribePromise = new Promise((resolve, reject) => {
                exec(`python "${scriptPath}" "${audio_url}"`, (error, stdout, stderr) => {
                    if (error) {
                    ...
                    const parsedLyrics = lines.map((line, index) => {
                        ...
                        if (match) {
                            ...
                            return { id: index, text: content, start: startTime, end: 0 }; // End calculated later
                        }
                        return null;
                    }).filter(item => item !== null);

                    // Calculate end times
                    const finalLyrics = parsedLyrics.map((item, i) => {
                        const nextItem = parsedLyrics[i + 1];
                        const endTime = nextItem ? nextItem.start : (item.start + 2);
                        return { ...item, end: endTime };
                    });
            ...
            if (!lyrics) {
                ...
                currentData.lyrics = lyrics;

                await db.execute(
                    'UPDATE projects SET data = ? WHERE id = ?',
                    [JSON.stringify(currentData), req.params.id]
                );
            }
        ...
        res.json({ message: 'Audio uploaded', audio_url, lyrics });
    } catch (err) {
